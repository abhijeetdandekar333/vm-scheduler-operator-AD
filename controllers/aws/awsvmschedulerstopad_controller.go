...
// AWSVMSchedulerStopadReconciler reconciles a AWSVMSchedulerStopad object
type AWSVMSchedulerStopadReconciler struct {
	Client client.Client
	Scheme *runtime.Scheme
	Log    logr.Logger
}

// +kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstopads,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstopads/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstopads/finalizers,verbs=update
...
func (r *AWSVMSchedulerStopadReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := ctrllog.FromContext(ctx)
	//log := r.Log.WithValues("AWSVMScheduler", req.NamespacedName)
	log.Info("Reconciling AWSVMSchedulerStopad")
	...
	awsVMScheduler := &awsv1.AWSVMSchedulerStopad{}
	...
	if err != nil && errors.IsNotFound(err) {
		...
		cron := r.cronJobForAWSVMSchedulerStopad(awsVMScheduler)
...
func (r *AWSVMSchedulerStopadReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&awsv1.AWSVMSchedulerStopad{}).
		Owns(&batchv1.CronJob{}).
		Complete(r)
}

// CronJob Spec
func (r *AWSVMSchedulerStopadReconciler) cronJobForAWSVMSchedulerStopad(awsVMScheduler *awsv1.AWSVMSchedulerStopad) *batchv1.CronJob {

	cron := &batchv1.CronJob{
		ObjectMeta: v1.ObjectMeta{
			Name:      awsVMScheduler.Name,
			Namespace: awsVMScheduler.Namespace,
			Labels:    AWSVMSchedulerStopadLabels(awsVMScheduler, "awsVMScheduler"),
		},
...
func AWSVMSchedulerStopadLabels(v *awsv1.AWSVMSchedulerStopad, tier string) map[string]string {
	return map[string]string{
		"app":                     "AWSVMSchedulerStopad",
		"AWSVMSchedulerStopad_cr": v.Name,
		"tier":                    tier,
	}
}
