...
// AWSVMSchedulerStartadReconciler reconciles a AWSVMSchedulerStartad object
type AWSVMSchedulerStartadReconciler struct {
	Client client.Client
	Scheme *runtime.Scheme
	Log    logr.Logger
}

// +kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstartads,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstartads/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstartads/finalizers,verbs=update
// +kubebuilder:rbac:groups=batch,resources=cronjobs;jobs,verbs=get;list;watch;create;update;patch;delete

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the AWSVMSchedulerStartad object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.13.0/pkg/reconcile
func (r *AWSVMSchedulerStartadReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := ctrllog.FromContext(ctx)
	//log := r.Log.WithValues("AWSVMScheduler", req.NamespacedName)
	log.Info("Reconciling AWSVMSchedulerStartad")
	...
	awsVMScheduler := &awsv1.AWSVMSchedulerStartad{}
	...
	if err != nil && errors.IsNotFound(err) {
		...
		cron := r.cronJobForAWSVMSchedulerStartad(awsVMScheduler)
...
func (r *AWSVMSchedulerStartadReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&awsv1.AWSVMSchedulerStartad{}).
		Owns(&batchv1.CronJob{}).
		Complete(r)
}

// CronJob Spec
func (r *AWSVMSchedulerStartadReconciler) cronJobForAWSVMSchedulerStartad(awsVMScheduler *awsv1.AWSVMSchedulerStartad) *batchv1.CronJob {

	cron := &batchv1.CronJob{
		ObjectMeta: v1.ObjectMeta{
			Name:      awsVMScheduler.Name,
			Namespace: awsVMScheduler.Namespace,
			Labels:    AWSVMSchedulerStartadLabels(awsVMScheduler, "awsVMScheduler"),
		},
...
func AWSVMSchedulerStartadLabels(v *awsv1.AWSVMSchedulerStartad, tier string) map[string]string {
	return map[string]string{
		"app":                      "AWSVMSchedulerStartad",
		"AWSVMSchedulerStartad_cr": v.Name,
		"tier":                     tier,
	}
}
